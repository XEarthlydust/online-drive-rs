...
pub fn all_router() -> Router {
    Router::with_path("api").push(
        Router::with_path("file")
            .push(Router::with_path("upload-by-hash").hoop(auth_middleware).put(upload_by_hash))
            .push(Router::with_path("start-upload").hoop(auth_middleware).put(start_upload_file))
            .push(Router::with_path("upload-part").hoop(auth_middleware).put(upload_file_part))
            .push(Router::with_path("finish-upload").hoop(auth_middleware).post(finish_upload))
