...
use rbatis::rbdc::rt::tokio::signal;
...
async fn main() {
    ...
    let acceptor = TcpListener::new(format!("{}:{}", config.port.bind_ip, config.port.share))
        .bind()
        .await;
    ...
    connect_nacos(env!("CARGO_PKG_NAME"), "", config!(), config!().port.share).await;
